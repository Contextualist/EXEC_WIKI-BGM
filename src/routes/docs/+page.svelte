...
<article>
	...
	<section>
		...
		<section>
			<h3>功能区与工作流概览</h3>
			{@render pic('/docs/tutorial-0.webp', '功能分区，预览和大致流程')}
		</section>
		<section>
			<h3>编写音乐条目</h3>
			<ol>
				<li>
					找到官方可供复制的曲目制作人员信息，一般会在特设网站页面上、YouTube/SoundCloud 上 XFD
					的描述中、Booth/MelonBooks 的商品描述中（如下图所示）。又或在如 MusicBrainz
					的第三方音乐维基站中找到并验证<sup
						><Tooltip
							title={`维基站上的信息可能有错漏，使用前最好与官方信息核对。<br />
							官方信息来源有：小册子/小册子扫图、官方页面/官方商店页面
							`}
							safeHTML={true}
						/></sup
					>曲目信息。
				...
				<li>
					复制曲目信息到左侧的编辑器里，或使用「导入…」从维基站导入，然后根据高亮和右侧的生成预览做调整：
				</li>
				<blockquote>
					什么样的信息可以被识别呢？简单来说：<b>可关联的职位</b><sup
						><Tooltip
							title={`目前班古米对音乐专辑支持的可关联职位有：<br />
							艺术家、制作人、作曲、作词、编曲、插图、脚本、录音、厂牌、出版方、原作、声乐、乐器、混音、母带制作。<br />
							编辑器能识别这些职位的常见中日英表记方式。
							`}
							safeHTML={true}
						/></sup
				...
				<li>
					右上角的关联概览中，已经在关联库里匹配到的人物会显示头像。而没匹配到的会是这种暗红色的首字母空格子，点击以搜索并添加到关联库。
				</li>
				{@render pic('/docs/tutorial-3b.webp', '关联概览')}
				<li>
